 ...
 * @param isTapStack Whether the stack trace is in tap's assert format
...
exports.getDeepestCallInfo = function (stack, isTapStack) {
    var matches;
    if (isTapStack)
    ...
    if (matches && !_.isUndefined(matches[2])) {
        try {
...
exports.truncateTrace = function (stackHolder, atPath) {
    ...
    if (droppedDeepestCall && !_.isUndefined(stackHolder.source)) {
        var callInfo = exports.getDeepestCallInfo(stack, true);
