...
The 'options' parameter of the constructor accepts the following values:
 ...
 - colorOverrides: object mapping style names to color escape sequences that are to override the default styles for those names
 - continuation: string of characters to place at the start of each continuation line of a line that is being wrapped at a width, except when overridden by a non-null lineContinuation parameter (defaults to empty string)
 - writeFunc: function(line) to call for writing lines, or null (optional)
 
LineMaker preconfigures styles named 'bold', 'reverse', 'normal', and 'underline', which correspond to standard escape sequences.
******************************************************************************/
...
function LineMaker(options) {
    ...
    if (this._styleMode > LineMaker.STYLE_MONOCHROME) {
        ...
        if (options.colorOverrides) {
            for (var styleName in options.colorOverrides)
                this._styleMap[styleName] = options.colorOverrides[styleName];
        }
    }
 ...
 * @param lineStart String with which to start each line prior to line-wrapping, or a function(lineIndex) that takes the zero-based index of a line in text and returns the lineStart string to use for the line. (defaults to '')
 * @param lineContinuation String with which to start each wrapped segment of a line, or a function(lineIndex) that takes the zero-based index of a line in text and returns the lineContinuation string to use for the line. (defaults to the configured continuation value)
 * @param firstIndent Number of characters in indentation of first line relative to left margin. If non-zero, the background highlight on the last line of the wrap will end at the end of the text. (defaults to 0)
 */

LineMaker.prototype.colorWrap = function (
        styleID, text, width, firstIndent, lineStart, lineContinuation)
{
    if (this._styleMode <= LineMaker.STYLE_MONOCHROME)
        styleID = null;
    return this._wrap(styleID, text, width, firstIndent, lineStart,
            lineContinuation);
};
...
LineMaker.prototype.style = function (styleID, text) {
    if (this._styleMode === LineMaker.STYLE_OFF)
        return text;
    var esc = this._styleMap[styleID];
    if (esc === '')
        return text;
    return esc + text + this._styleMap.normal;
};
        ...
        styleID, text, width, firstIndent, lineStart, lineContinuation)
{
    ...
    lineStart = lineStart || '';
    var lineStartFunc = lineStart;
    if (typeof lineStart === 'string')
        lineStartFunc = function (lineIndex) { return lineStart; }
    lineContinuation = lineContinuation || this._continuation;
    var lineContFunc = lineContinuation;
    if (typeof lineContinuation === 'string')
        lineContFunc = function (lineIndex) { return lineContinuation; }
    ...
    for (var lineIndex = 0; lineIndex < lines.length; ++lineIndex) {
        line = lineStartFunc(lineIndex);
        if (split && split.carryOverEsc)
            line += split.carryOverEsc;
        ...
        while (line !== null) {
            ...
            if (width > 0) {
                ...
                if (remainder !== null)
                    remainder = lineContFunc(lineIndex) + remainder;
                firstIndent = 0;
            }
            if (esc)
