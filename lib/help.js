...
line("Subtap also provides the folling options. The '-nN' and '-bN' options most distinguish subtap from other 'tap' test runners. They allow the user to quickly isolate and focus on problematic root subtests.");
blankLine();
group = [];
group.push(['-b', "Bail out of testing on the first assertion to fail. (Same as '-b' option in the 'tap' runner.)"]);
group.push(['-bN', "Bail out of testing after the Nth failing root subtest."]);
group.push(['-c0', "Render output without color, emphasis, or other ANSI terminal codes."]);
group.push(['-c1', "Render output in monochrome, allowing emphasis."]);
group.push(['-c2', "Render multicolor output. (default)"]);
group.push(['-e', "Catch and report subtest exceptions as failed assertions. Root test exceptions always terminate the run because they interrupt the numbering of root subtests."]);
group.push(['-f', "Include source code of functions in found/wanted values."]);
group.push(['-h', "Show this help information. (same as --help)"]);
group.push(['-nN', "Only run the Nth root subtest. Subtap displays root subtest numbers as the tests run for referencing."]);
group.push(['-tN', "Timeout for inactivity after N millisecs. To disable timeout, set N to 0. (default 3000)"]);
group.push(['--diffs [<flags>]', "Indicate the differences between found and wanted values according to these flags. To suppress all indications, provide no flags. (default BCU)\n\n"+
...
group.push(['--help', "Show this help information. (same as -h)"]);
