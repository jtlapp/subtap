...
blankLine();
line("SUBTAP is a test runner for debugging test suites by selectively running subtests. It is also a TAP pretty-printer that emphasizes making differences between found and wanted values obvious at a glance. This tool only works with tests that employ the 'tap' module (github 'node-tap').");
blankLine();
...
line("Subtap also provides the following options. The -bN and -r options most distinguish subtap from other 'tap' test runners. They allow the user to quickly isolate and focus on problematic root subtests.");
blankLine();
group = [];
...
group.push(['--mark=<f>[:<g>]', "Mark differences between found & wanted values according to flags. --mark=<f> sets flags <f> for all difference comparisons. --mark=<f>:<g> sets flags <f> for comparing consecutive values and flags <g> for comparing adjacent diff lines (see -d, --diff). (default --mark=BCR:BC)\n\n"+
...
group.push(['SUBTAP_COLOR', "Path to a YAML file specifying color overrides. The path may be relative to the current working directory. The file associates the following style names with ANSI escape code strings: (e.g. To make secondary labels orange, write 'label: \"\\e[38;5;166m\"')\n\n"+
