...
var COLOR_MAP_256 = {
    'fail': '\x1b[31m', // dark red
    'fail-emph': '\x1b[97m\x1b[101m', // bright white on bright red
    //'fail-emph': '\x1b[48;5;210m', // on light red
    'found': '\x1b[48;5;229m', // light yellow
    'pass': '\x1b[32m', // dark green
    'wanted': '\x1b[48;5;045m' // light blue
};
...
AbstractReport.prototype.bailout = function (reason, counts) {
    this._failedClosing(counts);
    var line = this._format.red(BULLET_FAIL +" BAIL OUT! "+ reason);
...
AbstractReport.prototype._formatName = function (bullet, nameParse, color) {
    var text = this._format.bold(bullet +" "+ nameParse.name);
    if (color)
        text = this._format[color].call(this._format, text);
    if (nameParse.file) {
        var file = nameParse.file; // includes preceding space
        if (color)
            file = this._format[color].call(this._format, file);
        text += file;
    }
    return text;
};
...
AbstractReport.prototype._printDiffs = function (indentLevel, assert) {
            ...
            wanted.indexOf("\n") === -1)
    {
        ...
        if (found.length < singleLineWidth)
            found += ' ';
        var foundHighlight = this._format.yellowBkg(found);
        this._printLine(indentLevel, 'found:  '+ foundHighlight);

        wanted = ' '+ wanted;
        if (wanted.length < singleLineWidth)
            wanted += ' ';
        var wantedHighlight = this._format.cyanBkg(wanted);
        ...
        var foundHighlight = this._format.yellowBkg(found, multilineWidth);
        this._printLine(indentLevel, 'found: >');
        this._print(this._format.multiline(indentLevel + 1, foundHighlight));
        
        var wantedHighlight = this._format.cyanBkg(wanted, multilineWidth);
...
AbstractReport.prototype._printFailedAssertion = function (assert, tweakFunc) {
    var indentLevel = this._printer._testStack.length;
    var line = "not ok "+ assert.id +" - "+ assert.name;
    if (assert.time)
        line += " # time="+ assert.time +"ms";
    if (tweakFunc)
        line = tweakFunc(line);
    line = this._format.bold(this._format.red(BULLET_FAIL +" "+ line));
...
FullReport.prototype.assertionFailed = function (assert) {
    ...
    this._printFailedAssertion(assert);
};

FullReport.prototype.assertionPassed = function (assert) {
    this._printTestContext();
    var text = BULLET_PASS +" ok "+ assert.id +" - "+ assert.name;
...
RootTestReport.prototype.assertionFailed = function (assert) {
    if (this._printer._testStack.length > 0) {
        if (!this._rootFailed) {
            ...
            var text = this._format.red(this._format.bold(BULLET_FAIL)) +' ';
            text += this._format.redBkg(this._format.bold(
                        this._format.white(nameParse.name)));
            if (nameParse.file)
                text += this._format.redBkg(this._format.white(nameParse.file));
            this._printLine(0, text);
            this._depthShown = 1;
        }
    ...
    var self = this;
    this._simplifyAssertion(assert);
    this._printFailedAssertion(assert, function(text) {
        if (self._printer._testStack.length === 0)
            return self._format.redBkg(self._format.white(text));
        return text;
    });
};
...
RootTestReport.prototype.closeTest = function (results) {
    ...
    if (this._printer._testStack.length === 1 && !this._rootFailed) {
        this._printUpLine();
        var nameParse = this._printer._testStack[0];
        var formattedName = this._formatName(BULLET_PASS, nameParse, 'green');
